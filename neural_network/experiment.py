...
class NeuralNetwork:
    # starting learning rate, it gets increased by LEARNING_STEP until error increases, then previous rate is used
    LEARNING_RATE = 0.3
    LEARNING_STEP = 0.015
    ...
    def batch_learning_train(self, training_inputs, target_outputs):
        training_sets = [] # for calculating total error
        for i in range(len(training_inputs)):
            training_sets.append([training_inputs[i], target_outputs[i]])
        ...
        for i in range(self.num_layers):
            for j in range(len(self.layers[i].neurons)):
                for k in range (len(self.layers[i].neurons[j].inputs)):
                    # keep increasing learning rate by 1% as long as total error keeps diminishing
                    learning_rate = self.LEARNING_RATE
                    total_error_BEFORE_weight_change = self.calculate_total_error(training_sets)
                    while True:
                        self.layers[i].neurons[j].weights[k] = self.layers[i].neurons[j].weight_adjustment_value[k] * learning_rate
                        total_error_AFTER_weight_change = self.calculate_total_error(training_sets)
                        #print(total_error_BEFORE_weight_change, total_error_AFTER_weight_change, learning_rate)
                        if total_error_AFTER_weight_change < total_error_BEFORE_weight_change and learning_rate < 1 :
                            learning_rate += self.LEARNING_STEP
                            total_error_BEFORE_weight_change = total_error_AFTER_weight_change
                        else:
                            #print("break", i, learning_rate)
                            learning_rate -= self.LEARNING_STEP
                            self.layers[i].neurons[j].weights[k] = self.layers[i].neurons[j].weight_adjustment_value[k] * learning_rate
                            break
            ...
            for j in range(len(self.layers[self.num_layers-1].neurons)):
                ...
                for i in range(self.layers[self.num_layers-1].neurons[j].num_inputs):
                    ...
                    while True:
                        ...
                        self.layers[self.num_layers-1].neurons[j].last_weight_change = learning_rate * ( delta * self.layers[self.num_layers-1].neurons[j].inputs[i] )  + self.INERTION_RATE * self.layers[self.num_layers-1].neurons[j].last_weight_change
                        self.layers[self.num_layers-1].neurons[j].weights[i] += self.layers[self.num_layers-1].neurons[j].last_weight_change
                        total_error_AFTER_weight_change = self.calculate_total_error(training_sets)
                        # undo change after calculating error
                        self.layers[self.num_layers-1].neurons[j].weights[i] -= self.layers[self.num_layers-1].neurons[j].last_weight_change
                        if total_error_AFTER_weight_change < total_error_BEFORE_weight_change and learning_rate < 1:
                            learning_rate += self.LEARNING_STEP
                        else:
...
for name in names:
    for i in range(1,21):
        ...
        outputs.append(out)
        train_sets.append([inp, out])
        
# prepare data as minibatches
if sys.argv[1] == "minibatches":     
...
while True:
    ...
    elif training_algorithm == "minibatch":
        for i in range(len(batch_inputs)):
            nn.batch_learning_train(batch_inputs[i], batch_outputs[i])
    else:
        print("Usage: python3 neural-network.py [batch|online|minibatches] hidden_layer1_num ... output_layer_num ")
        exit()
    total_error = nn.calculate_total_error(train_sets)
